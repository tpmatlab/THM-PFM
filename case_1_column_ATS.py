...
def NonlinearDisplacementSolver():
    ...
    Res_u = inner(grad(v), sigma(u, phi_n, P_v_theta, T_theta)) * dx
...
def LinearPhaseFieldSolver():
    ...
    solver_u : NonlinearVariationalSolver object
        The nonlinear solver for the displacement problem
    '''
    Res_phi = (Gc(T_theta) * l_0 * inner(grad(phi_hat), grad(q_phi)) +
               ((Gc(T_theta) / l_0) + 2.0 * H(u, P_v_theta, T_theta, Hold)) * inner(phi_hat, q_phi) -
               2.0 * H(u, P_v_theta, T_theta, Hold) * q_phi) * dx
