from dolfin import *
from material_properties_biot_0 import *
...
# Name of the case
case = f'case_3_wall_uniform_biot_0_ATS'
...
copy('./bazant_properties_biot_0.py', dir_case + '/' + case + '_properties_backup.py')
...
T_flame = Expression('T_t',
                     degree=1, T_t=T_huc(0))

# Thermohygro problem Dirichlet boundary conditions (BC)
bc_TH_l = DirichletBC(V.sub(1), T_flame, left)
bcs_TH = [bc_TH_l]
...
# bc_b = DirichletBC(W, (Constant(0.), Constant(0.)), bottom)
...
while t <= T_total:
    ...
    while theta < 1:
        ...
        while (err_max > tol) and (i <= i_max):
            # 4.8.1. Solve the displacemnet problem
            solver_disp.solve()
