...
a_fit = 1.1236852912742759
b_fit = 1.0226369553980956
c_fit = 0.18748643177058377
d_fit = -12.618378938938028
e_fit = 8.541556579223903
f_fit = -0.49408552298142344
g_fit = 0.3661528450874246


T_flame = Expression('((a * pow((-((x[1] / ly - b) / c)),(a - 1))' + 
                     '* exp(-pow(-((x[1] / ly - b) / c), a))) / c' +
                     '+ d * pow(x[1] / ly, 3) + e * pow(x[1] / ly, 2)' +
                     '+ f * x[1] / ly + g) * T_t + T_0',
                     degree=1, a=a_fit, b=b_fit, c=c_fit, d=d_fit,
                     e=e_fit, f=f_fit, g=g_fit,
                     ly=ly, T_0=T_0, T_t=T_huc(0))
...
ResT += (B_t * (T - T_flame) + epsilon_emi * sigma_SB * (T_n**3 * T - T_flame**4)) * q_T * ds_heated
