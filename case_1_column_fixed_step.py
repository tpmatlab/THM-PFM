...
# Name of the case
case = f'case_1_column_fixed_step'
...
while t <= T_total:
    ...
    # 4. Solve the displacement/phase-field problem
    n_DPF = 1
    n_DPF_max = 20
    err_max = 1
    while (err_max > tol) and (n_DPF <= n_DPF_max):
        # 4.1. Solve the displacement problem
        solver_disp.solve()
        # 4.2. Solve phase-field problem
        solver_phi.solve()
        # 4.3. Calculate the error between current and previous displacement
        err_u = assemble(dot(u - u_n, u - u_n) * dx)**0.5
        # 4.4. Calculate the error between current and previous phase-field
        err_phi = assemble((phi - phi_n)**2 * dx)**0.5
        # 4.5. Calculate the maximum error
        err_max = max(err_u, err_phi)
        # 4.6. Update the variables
        u_n.vector()[:] = u.vector()
        phi_n.vector()[:] = phi.vector()
        Hold.assign(project(H(u, P_v, T, Hold), V_H))
        # 4.7. Print the information
        print(f'    | ---------------------------------------------------- |')
        print(f'    | ---------------------------------------------------- |')
        print(f'    |       t = {t/3600:6.3f}h          |      n_DPF = {n_DPF:4g}       |')
        print(f'    |       err_u = {err_u:.3e}    |  err_phi = {err_phi:.3e}    |')

        # 4.8. Increase the current iteration number
        n_DPF += 1

    # 5. Write the data to the csv
    writer.writerow([t, n_TH, n_DPF, err_u, err_phi])
    ...
    if (nt % freq_out == 0):
        ...
        filex.write(u, t)
        filex.write(phi, t)
        sig_PS = project(sigma_PS(u, phi, P_v, T), V_phi)
        sig_PS.rename('Maximum Principal Stress [MPa]', 'sig_PS')
        filex.write(sig_PS, t)

    # 7. Print the information
    msg = f'% Complete: {t / T_total * 100:.2f}'
    print(f'    | {msg:^52} |')

    # 8. Increase the current time step number
    nt += 1
    # 9. Increase the current time
    t += dt

# 10. End loop over time steps
file.close()
